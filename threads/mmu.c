 ...
 * PML4E가 VADDR에 대한 페이지 테이블을 가지고 있지 않으면, 동작은 CREATE에 따라 다릅니다.
 * CREATE가 true이면, 새로운 페이지 테이블이 생성되고 그에 대한 포인터가 반환됩니다.
...
/* 새로운 페이지 맵 레벨 4 (pml4)를 생성하며, 커널 가상 주소에 대한 매핑을 가지고 있지만,
 * 사용자 가상 주소에 대한 매핑은 없습니다.
 * 새로운 페이지 디렉토리를 반환하거나, 메모리 할당이 실패하면 null 포인터를 반환합니다. */
uint64_t *
pml4_create(void)
{
...
/* FUNC를 사용 가능한 각 pte 항목에 적용합니다. 커널의 항목도 포함됩니다. */
...
void pml4_destroy(uint64_t *pml4)
{
	...
	/* PML4 (vaddr)가 1 이상이면, 정의에 따라 커널 공간입니다. */
...
/* 페이지 디렉토리 PD를 CPU의 페이지 디렉토리 기본 레지스터에 로드합니다. */
 ...
 * 해당 물리적 주소에 해당하는 커널 가상 주소를 반환하거나, UADDR가 매핑되지 않은 경우 null 포인터를 반환합니다. */
void *
...
/* 사용자 가상 페이지 UPAGE를 커널 가상 주소 KPAGE로 식별된 물리적 프레임에 매핑을 추가합니다.
 * UPAGE는 이미 매핑되어 있지 않아야 합니다. KPAGE는 아마도 palloc_get_page()로 얻은 페이지일 것입니다.
 * WRITABLE이 true이면, 새로운 페이지는 읽기/쓰기가 가능하며; 그렇지 않으면 읽기 전용입니다.
 * 성공하면 true를 반환하고, 메모리 할당이 실패하면 false를 반환합니다. */
 ...
 * 이후 페이지에 대한 접근은 오류를 발생시킵니다. 페이지 테이블 항목의 다른 비트는 보존됩니다.
 * UPAGE는 매핑될 필요가 없습니다. */
...
/* PML4에서 가상 페이지 VPAGE에 대한 PTE가 더럽혀졌다면 true를 반환합니다.
 * 즉, PTE가 설치된 이후 페이지가 수정되었는지 여부입니다.
 * VPAGE에 대한 PML4에 PTE가 없으면 false를 반환합니다. */
...
/* PML4에서 가상 페이지 VPAGE에 대한 PTE의 더럽혀진 비트를 DIRTY로 설정합니다. */
 ...
 * 즉, PTE가 설치된 시점과 마지막으로 지워진 시점 사이에 접근되었는지 여부입니다.
 * VPAGE에 대한 PML4에 PTE가 없으면 false를 반환합니다. */
...
/* PD에서 가상 페이지 VPAGE에 대한 PTE의 접근 비트를 ACCESSED로 설정합니다. */
